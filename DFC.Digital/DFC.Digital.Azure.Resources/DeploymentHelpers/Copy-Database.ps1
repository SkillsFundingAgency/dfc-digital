...
try {
    $azurepsver = ((Get-Module AzureRM -ListAvailable | Sort-Object { $_.Version.Major } -Descending).Version.Major)[0]
    ...
    if ($azurepsver -lt 6) {
        $ServerResource = Find-AzureRmResource -ResourceNameEquals $ServerName -ResourceType "Microsoft.Sql/servers" -ErrorAction SilentlyContinue
    } else {
        $ServerResource = Get-AzureRmResource -Name $ServerName -ResourceType "Microsoft.Sql/servers" -ErrorAction SilentlyContinue
    }
    if (!$ServerResource) {
