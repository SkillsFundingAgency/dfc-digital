...
namespace DFC.Digital.Repository.SitefinityCMS.Modules
{
    public class DynamicContentConverter : IDynamicModuleConverter<JobProfileMessage>
    {
        ...
        public JobProfileMessage ConvertFrom(DynamicContent content)
        {
            ...
            {
                JobProfileId = dynamicContentExtensions.GetFieldValue<Guid>(content, content.ApprovalWorkflowState == Constants.WorkflowStatusDraft ? Constants.ContentId : Constants.OriginalContentId),
        ...
        private IEnumerable<FrameworkSkillItem> GetRelatedSkillsData(DynamicContent jobProfileContent, DynamicContent content, string relatedField)
        {
            ...
            if (relatedItems != null)
            {
                foreach (var relatedItem in relatedItems)
                {
                    ...
                    {
                        Id = dynamicContentExtensions.GetFieldValue<Guid>(relatedItem, jobProfileContent.ApprovalWorkflowState == Constants.WorkflowStatusDraft ? Constants.ContentId : Constants.OriginalContentId),
        ...
        private IEnumerable<RestrictionItem> GetRestrictions(DynamicContent content, string relatedField)
        {
            ...
            if (relatedItems != null)
            {
                foreach (var relatedItem in relatedItems)
                {
                    ...
                    {
                        Id = dynamicContentExtensions.GetFieldValue<Guid>(relatedItem, relatedItem.ApprovalWorkflowState == Constants.WorkflowStatusDraft ? Constants.ContentId : Constants.OriginalContentId),
        ...
        private SocCodeItem GenerateSocData(DynamicContent jobprofileContent, DynamicContent content)
        {
            ...
            {
                Id = dynamicContentExtensions.GetFieldValue<Guid>(content, jobprofileContent.ApprovalWorkflowState == Constants.WorkflowStatusDraft ? Constants.ContentId : Constants.OriginalContentId),
        ...
        private IEnumerable<WYDRelatedContentType> GetWYDRelatedItems(DynamicContent content, string relatedField)
        {
            ...
            if (items != null && items.Any())
            {
                foreach (var item in items)
                {
                    ...
                    {
                        Id = dynamicContentExtensions.GetFieldValue<Guid>(item, content.ApprovalWorkflowState == Constants.WorkflowStatusDraft ? Constants.ContentId : Constants.OriginalContentId),
